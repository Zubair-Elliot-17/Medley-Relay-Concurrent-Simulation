...
public class MedleySimulation {
	...
	static PeopleLocation [] peopleLocations;  //array to keep track of where swimmers are
	...
	static CountDownLatch latch = new CountDownLatch(1); // Latch to synchronize the start of the race
	...
	public static void setupGUI(int frameX,int frameY) {
		...
		startB.addActionListener(new ActionListener() {
		    public void actionPerformed(ActionEvent e)  {
					   latch.countDown(); // starts procedure of entering stadium
		    }
		   });
...
//Main method - starts it all
	public static void main(String[] args) throws InterruptedException {
		...
		Semaphore[] teamSemaphores = new Semaphore[numTeams];
		for (int i = 0; i < numTeams; i++) {
            teamSemaphores[i] = new Semaphore(1, true); // Assuming one permit for simplicity
        }
      // ensures that the race starts when all backstroke swimmers are ready
		CyclicBarrier barrier = new CyclicBarrier(10, new Runnable() {
    		public void run() {
      			  System.out.println("All backstroke swimmers are in place. The race can now start!");
    		}
		});

	    finishLine = new FinishCounter(); //counters for swimmers inside and outside club
		...
		for (int i=0;i<numTeams;i++) {
        	teams[i]=new SwimTeam(i, finishLine, peopleLocations , teamSemaphores[i] , barrier );        	
		}
